...
{
	const struct machine_desc *mdesc;//设备描述符结构体

	setup_processor();//再次检测处理器类型，并初始化处理器相关的底层变量，内核启动时处理器信息就是通过该函数打印
	//将之前在head.S中检查的Machine ID推迟到这里	
...
//init_mm部分数据初始化
	...
	*cmdline_p = cmd_line;
	...
	parse_early_param();
									/* 分析boot_command_line（内核启动参数字符串）中的数据，
	...
	if (mdesc->init_early)
		mdesc->init_early();/*在mach-exynos5-dt.c中定义init_eraly为exynos_firmware_init*/
}
