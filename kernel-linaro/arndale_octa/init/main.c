...
static noinline void __init_refok rest_init(void)
{
	...
	rcu_read_unlock();
	complete(&kthreadd_done);//通过complete通知kernel_init线程kthread线程已经创建成功
	...
	schedule_preempt_disabled();//使能抢占，但不重新调度
...
/*内核的第一个C函数,在函数__mmap_switched中调用，r9存放的是processor ID，寄存器r1存放的是machine type,r2存放的是atags pointer*/
asmlinkage void __init start_kernel(void)
{
	 ...
	 * lockdep hash:
	 */
	lockdep_init();//内核调试模块，用于检测内核互斥机制潜在的死锁问题
	smp_setup_processor_id();//针对SMP处理器，如果不是则是弱引用函数
	debug_objects_early_init();//调试对象的早期初始化
 ...
 */
	...
	setup_nr_cpu_ids();//以下三个函数针对SMP处理器
	setup_per_cpu_areas();
	...
	trap_init();//对内核陷阱异常进行初始化，ARM架构中为空函数
	 ...
	 */
	preempt_disable();//禁止抢占和中断，早期启动时期，调度比较脆弱
	if (WARN(!irqs_disabled(), "Interrupts were enabled *very* early, fixing it\n"))
		local_irq_disable();
	idr_init_cache();//为IDR机制分配缓存
	rcu_init();//内核RCU机制初始化
	tick_nohz_init();//动态时钟框架初始化
...
//检查initrd的位置是否符合要求
	...
	rest_init();//剩余的初始化，主要是创建并启动内核线程init
}
...
/*内核init线程运行函数,完成设备驱动程序的初始化，并调用init_pos函数启动用户空间的init进程*/
static int __ref kernel_init(void *unused)
{
	...
	if (!try_to_run_init_process("/sbin/init") ||
		...
		return 0;
//检测完ramdisk_execute_command和execute_command为空的情况下，顺序执行四个初始化程序
//如果都没有找到就打印出错信息，出现该错误的可能原因是:
//1.启动参数配置有问题:指定了init进程，但是没有找到，默认四个程序不在文件系统中
//2.文件系统挂载有问题
//3.init程序无执行权限
...
{
	/*
	 * Wait until kthreadd is all set-up.在这里挂起等待kthreadd线程结束
	 */
	...
	//1.制作根文件系统时忘记创建/dev/console节点
	//2.文件系统挂载问题，挂载的文件系统不是什么都没有就是挂错了节点
	if (sys_open((const char __user *) "/dev/console", O_RDWR, 0) < 0)
		pr_err("Warning: unable to open an initial console.\n");

	//复制两次标准输入(0),一个作为标准输出(1)，一个作为标准错误(2)；完成后，标准输入/输出/错误都是/dev/console
	(void) sys_dup(0);
	(void) sys_dup(0);
	/*
