...
static void boot_jump_linux(bootm_headers_t *images)
{
	...
	kernel_entry = (void (*)(int, int, uint))images->ep;/*将image(内核)的入口点赋值给kernel_entry*/
	...
	announce_and_cleanup();/*打印"starting kernel..，并清除环境(关闭中断，关闭I/D Cache)"*/
...
#endif
		r2 = gd->bd->bi_boot_params;/*存放的是需要传递给内核的参数*/
